                ...
                <Table stickyHeader aria-label="sticky table" size="small">
                    ...
                    <TableHead>
                        <TableRow>
                            {props.columns.map((column) => (
                                <TableCell key={column.dataField} align={column.align || 'left'} style={{ ...column.headerStyle }} sortDirection={props.searchGet.sort === column.dataField ? props.searchGet.order : false}>
                                    {column.sort ? (
                    ...
                    <TableBody>
                        {props.appdata.map((row) => (
                            <TableRow hover role="checkbox" tabIndex={-1} key={row[props.keyField]} onClick={(event) => handleRowClick(event, row)} sx={{ cursor: props.rowEvents ? 'pointer' : 'default' }}>
                                {props.columns.map((column) => {
